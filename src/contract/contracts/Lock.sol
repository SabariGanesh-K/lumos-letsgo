...
contract Lock {
    ...
    struct UserInfo {
        address wallet;
        uint256[] raiseIds; // not gonna work
        bool deactivated;
    ...
    function _checkDisputes(uint256 raiseId) private view returns (bool) {
        ...
        for (uint256 i = 0; i < raiseInfo[raiseId].transactions.length; i++) {
            if (
                ...
                1 days &&
                raiseInfo[raiseId].transactions[i].against_votes >=
    ...
    // function returnRaiseData() public view returns(Raise r){
    //     return raiseInfo[userInfo[msg.sender].currentRaise];
    // }
}
